(function () {
    ...
    var sdk = function (params) {
        ...
        var PRODUCT_CONFIG = params.PRODUCT_CONFIG;
        if (!PRODUCT_CONFIG) {
            window.console.error('请配置PRODUCT_CONFIG');
            return;
        }

        if (!params.TRACK_TYPE) {
            window.console.error('请配置TRACK_TYPE');
            return;
        }

        if (!params.TRACK_URL) {
            window.console.error('请配置TRACK_URL');
            return;
        }
                    ...
                    if (this.options.detectCamouflage) {
                        ...
                        if (this.options.useFeatures) {
                            ...
                            if (this.os.name === 'iOS' && this.os.version) {
                                ...
                                if (this.os.version.isOlder('5.0') && Boolean(window.Worker)) {
                            ...
                            if (this.os.name !== 'iOS' && this.browser.name === 'Safari' && this.browser.version) {
                                ...
                                if (this.browser.version.isOlder('4.0') && Boolean(window.applicationCache)) {
                                    this.features.push('foundAppCache');
                                    this.camouflage = true;
                                }

                                if (this.browser.version.isOlder('4.1') && Boolean(window.history && history.pushState)) {
                                    this.features.push('foundHistory');
                                    this.camouflage = true;
                                }

                                if (this.browser.version.isOlder('5.1') && Boolean(document.documentElement.webkitRequestFullScreen)) {
            ...
            if (uaData.device.type === 'mobile' || uaData.device.type === 'tablet') {
                ...
                // handle meizu
                    ...
                    if (match[3]) {
                        ...
                        uaData.device.model = String(tmpModel);
                    }
                ...
                // UIMI
                    ...
                    if (match[2]) {
                        ...
                        uaData.device.model = String(match[1]);
                    }
                }
                // eton
        ...
        var _ = {
            ...
            isArguments: function isArguments(obj) {
                return Boolean(obj && hasOwnProperty.call(obj, 'callee'));
            },
        ...
        _.register_event = function () {
            ...
            var register_event = function register_event(element, type, handler, oldSchool, useCapture) {
                ...
                if (element.addEventListener && !oldSchool) {
                    element.addEventListener(type, handler, Boolean(useCapture));
                } else {
                    var ontype = 'on' + type;
        ...
        _.info = {
            ...
            properties: function properties() {
                ...
                if (DEVICEINFO.browser.version != null) {
                    browserVer = DEVICEINFO.browser && DEVICEINFO.browser.version && DEVICEINFO.browser.version.original;
                }

                return {
                    // 设备型号
                    deviceBrand: DEVICEINFO.device && DEVICEINFO.device.manufacturer,
                    deviceModel: DEVICEINFO.device && DEVICEINFO.device.model || '',
                    // 操作系统
                    osType: DEVICEINFO.os && DEVICEINFO.os.name,
                    // 操作系统版本
                    osVer: DEVICEINFO.os && DEVICEINFO.os.version && DEVICEINFO.os.version.original || '',
                    // 设备平台
                    devicePlatform: DEVICEINFO.device && DEVICEINFO.device.type,
                    // 浏览器名称
                    browser: DEVICEINFO.browser && DEVICEINFO.browser.name,
                    // 浏览器版本
        ...
        _.ajax = {
            ...
            get: function get(url, options, callback) {
                try {
                    $.getJSON(url + '?data=' + options.data + '&jsoncallback=?', function (data) { });
                } catch (e) {
                }
            }
        };
        ...
        _.UUID = function () {
            return function () {
                return String(Date.now()) + '-' + Math.floor(1e7 * Math.random()) + '-' + Math.random().toString(16).replace('.', '') + '-' + String(Math.random() * 31242).replace('.', '').slice(0, 8);
            };
        }();
            ...
            _createClass(USER_TRACK, [{
                ...
                value: function _send_request(properties, callback) {
                    ...
                    _.sendRequest(url, track_type, {
                        ...
                        sdkToken: this.instance._get_config('sdkToken')
                    }, callback_fn);
                }
                /**
                 * 设置用户属性
        ...
        var EVENT_TRACK = function () {
                 ...
                 */
            {
                ...
                value: function _session(callback) {
                    var session_start_time = Number(this.instance.get_property('sessionStartTime')) / 1000;
                    var updated_time = Number(this.instance.get_property('updatedTime')) / 1000;
            ...
            },
            {
                ...
                value: function track(event_id, event_name, event_type, properties, callback) {
                    ...
                    console.log('自研埋点数据:', truncated_data);
                    var callback_fn = function callback_fn(response) {
                        callback(response, data);
                    };
                    ...
                    _.sendRequest(url, track_type, {
                        data: _.base64Encode(_.JSONEncode(truncated_data)),
                        sdkToken: _this.instance._get_config('sdkToken')
                    }, callback_fn);
                }
        ...
        var LOCAL_STORAGE = function () {
            ...
            function LOCAL_STORAGE(config) {
                ...
                if (_.isObject(local_storage)) {
                    this['name'] = local_storage['name'] || String(config['sdkToken']) + '_sdk';
